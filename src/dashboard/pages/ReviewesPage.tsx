...
export default function ReviewsPage() {
  ...
  return (
    <DashboardLayout>
      <div className="mb-8">
        <h1 className="text-white font-black text-2xl mb-1">Reviews</h1>
        <p className="text-sm" style={{ color: 'rgba(255,255,255,0.4)' }}>All customer reviews — public and private</p>
      </div>
      ...
      <div className="glass-card rounded-2xl overflow-hidden">
        <div className="flex flex-wrap items-center gap-3 px-6 py-4 border-b" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
  ...
  />
          ...
          <div className="flex items-center gap-1.5">
            ...
            {[null, 5, 4, 3, 2, 1].map(r => (
              <button key={String(r)} onClick={() => { setRFilter(r); setPage(1); }}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all duration-200 ${rFilter === r ? 'bg-blue-500 text-white' : 'hover:bg-white/5'}`}
                style={rFilter === r ? {} : { color: 'rgba(255,255,255,0.4)' }}>
                {r === null ? 'All' : `${r}★`}
              </button>
            ))}
          </div>
        ...
        <div className="divide-y" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  ...
                  <div className="flex gap-0.5">
                    {[...Array(5)].map((_,i) => <Star key={i} size={11} className={i < r.rating ? 'text-yellow-400 fill-yellow-400' : 'text-white/15'} />)}
                  </div>
                </div>
          ...
          {rows.length === 0 && <div className="px-6 py-12 text-center text-sm" style={{ color: 'rgba(255,255,255,0.3)' }}>No reviews match your filters.</div>}
        </div>

        {total > 1 && (
          <div className="flex items-center justify-between px-6 py-4 border-t" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
            <p className="text-xs" style={{ color: 'rgba(255,255,255,0.3)' }}>{filtered.length} results</p>
            <div className="flex gap-1">
              {Array.from({ length: total }, (_, i) => i + 1).map(p => (
                <button key={p} onClick={() => setPage(p)}
                  className={`w-8 h-8 rounded-lg text-xs font-medium transition-all duration-200 ${page === p ? 'bg-blue-500 text-white' : 'hover:bg-white/5'}`}
                  style={page === p ? {} : { color: 'rgba(255,255,255,0.4)' }}>{p}</button>
              ))}
            </div>
          </div>
        )}
