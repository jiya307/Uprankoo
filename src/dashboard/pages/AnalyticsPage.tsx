...
export default function AnalyticsPage() {
  return (
    <DashboardLayout>
      <div className="mb-8">
        <h1 className="text-black font-black text-2xl mb-1">Analytics</h1>
        <p className="text-sm" style={{ color: 'black' }}>Performance insights and trends</p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-5 mb-5">
        <div className="glass-card p-6 rounded-2xl">
          <div className="flex items-center gap-2 mb-1"><Activity size={15} className="text-blue-400" /><h3 className="text-black font-bold text-base">QR Scans — This Week</h3></div>
          <p className="text-xs mb-5" style={{ color: 'black' }}>Total: {MOCK_WEEKLY_SCANS.reduce((a,b)=>a+b,0)} scans</p>
          <BarChart data={MOCK_WEEKLY_SCANS} labels={DAYS} color="#3B82F6" height={150} />
        </div>
        <div className="glass-card p-6 rounded-2xl">
          <div className="flex items-center gap-2 mb-1"><TrendingUp size={15} className="text-green-400" /><h3 className="text-black font-bold text-base">Google Reviews — 12 Months</h3></div>
          <p className="text-xs mb-5" style={{ color: 'black' }}>Total: {MOCK_MONTHLY_REVIEWS.reduce((a,b)=>a+b,0)} reviews</p>
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        <div className="glass-card p-6 rounded-2xl">
          <div className="flex items-center gap-2 mb-5"><BarChart3 size={15} className="text-purple-400" /><h3 className="text-black font-bold text-base">Sentiment Split</h3></div>
          <DonutChart segments={[
            ...
            {[
              ...
              <div key={row.label} className="flex items-center gap-4">
                <div className="w-36 flex-shrink-0">
                  <p className="text-xs" style={{ color: 'black' }}>{row.label}</p>
                  <p className="text-black font-bold text-sm">{row.count.toLocaleString()}</p>
                </div>
                <div className="flex-1 h-3 rounded-full overflow-hidden" style={{ background: 'black' }}>
                  <div className="h-full rounded-full" style={{ width: `${row.pct}%`, background: row.color }} />
                </div>
                <span className="text-xs w-9 text-right" style={{ color: 'black' }}>{row.pct}%</span>
              </div>
            ))}
          </div>
        </div>
