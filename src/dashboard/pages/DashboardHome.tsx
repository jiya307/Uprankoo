...
export default function DashboardHome() {
  ...
  return (
    <DashboardLayout>
      <div className="mb-8">
        <h1 className="text-black font-black text-2xl mb-1">Good morning, {user?.name?.split(' ')[0]} 👋</h1>
        <p className="text-sm text-black ">{user?.businessName} · Dashboard overview</p>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-5 mb-8">
        <div className="lg:col-span-2 glass-card p-6 rounded-2xl">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-black font-bold text-base">Weekly QR Scans</h3>
              <p className="text-xs mt-0.5" style={{ color: 'black' }}>Last 7 days</p>
            </div>
            <div className="text-right">
              <p className="text-black font-black text-2xl">{MOCK_STATS.scansThisWeek}</p>
              <p className="text-green-400 text-xs">↑ 12% this week</p>
            </div>
          </div>
          <BarChart data={MOCK_WEEKLY_SCANS} labels={DAYS} color="black" height={140} />
        </div>

        <div className="glass-card p-6 rounded-2xl">
          <h3 className="text-black font-bold text-base mb-1">Rating Split</h3>
          <p className="text-xs mb-5" style={{ color: 'black' }}>Public vs. Private</p>
          <div className="space-y-3">
            {[
              { s: '5★', c: 185, col: 'green', p: 53 }, { s: '4★', c: 98, col: 'black', p: 28 },
              { s: '3★', c: 35, col: 'red', p: 10 }, { s: '2★', c: 20, col: 'black', p: 6 },
              { s: '1★', c: 9, col: 'red', p: 3 },
            ].map(row => (
              <div key={row.s} className="flex items-center gap-3">
                <span className="w-5 flex-shrink-0 text-xs" style={{ color: 'black' }}>{row.s}</span>
                <div className="flex-1 h-2 rounded-full overflow-hidden" style={{ background: 'lightblue' }}>
                  <div className="h-full rounded-full" style={{ width: `${row.p}%`, background:'green' }} />
                </div>
                <span className="w-6 text-right text-xs" style={{ color: 'black' }}>{row.c}</span>
              </div>
            ))}
          </div>
      ...
      <div className="glass-card rounded-2xl overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
          <h3 className="text-black font-bold text-base">Recent Reviews</h3>
          ...
          {recent.map(r => (
            <div key={r.id} className="flex items-start gap-4 px-6 py-4 hover:bg-white/2 transition-colors">
              <div className="w-9 h-9 rounded-xl flex items-center justify-center text-xs font-bold flex-shrink-0 text-blue-400"
                style={{ background: 'white', border: '1px solid rgba(59,130,246,0.2)' }}>
                {r.avatar}
              </div>
