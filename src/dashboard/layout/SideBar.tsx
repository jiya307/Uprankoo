  ...
  const content = (
    <div className="flex flex-col h-full"  style={{ background: '#FFFFFF' }}>
      ...
      <div className="flex items-center justify-between px-5 h-16 border-b" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
        <span className="font-black text-xl text-black">up<span className="gold-text">Ranko</span></span>
        <button onClick={onClose} className="lg:hidden text-black hover:text-black transition-colors p-1"><X size={18} /></button>
      </div>
          ...
          <NavLink key={to} to={to} end={exact}
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 group ${
                isActive ? 'text-black' : 'hover:bg-blue-500'
              }`
            }
            style={({ isActive }) => isActive
              ? { background: 'rgba(59,130,246,0.14)', border: '1px solid rgba(59,130,246,0.2)', color: 'black' }
              : { color: '#374151' }
            }
          >
            {({ isActive }) => (
              <>
                <Icon size={16} style={{ color: isActive ? '#60A5FA' : undefined }} className={isActive ? '' : 'text-black group-hover:text-black'} />
      ...
      {/* Logout */}
      <div className="px-3 pb-4 border-t pt-3" style={{ borderColor: '#374151' }}>
