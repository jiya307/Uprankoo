  ...
  return (
    <nav
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-400 ${
        scrolled
          ? 'bg-[#0A0808]/95 backdrop-blur-md border-b border-white/5 shadow-lg'
          : 'bg-transparent'
      }`}
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-10 h-16 flex items-center justify-between gap-8">
        ...
        <Link to="/" className="flex items-center gap-2.5 group flex-shrink-0">
          <img
            src={logo}
            alt="upRanko"
            className="w-9 h-9 rounded-full object-cover ring-1 ring-white/20 group-hover:ring-brand-gold-500/60 transition-all duration-300"
          />
      ...
      {menuOpen && (
        <div
          className="md:hidden px-6 py-6 flex flex-col gap-5 border-t border-white/5"
          style={{ background: '#0A0808/98', backdropFilter: 'blur(20px)' }}
        >
          {links.map((l) => (
            <NavLink
              key={l.to}
              ...
              className={({ isActive }) =>
                `text-base font-medium transition-colors ${isActive ? 'text-white' : 'text-white/60 hover:text-white'}`
              }
            >
              {l.label}
            </NavLink>
          ...
          {isAuthenticated ? (
            ...
            <>
              <Link to="/login" onClick={() => setMenuOpen(false)}
                className="text-base font-medium text-white/60 hover:text-white">Sign in</Link>
              <Link to="/register" onClick={() => setMenuOpen(false)}
                className="inline-flex items-center justify-center py-3 rounded-full text-sm font-bold text-white"
